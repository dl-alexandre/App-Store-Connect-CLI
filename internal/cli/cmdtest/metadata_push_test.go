...
func TestMetadataPushDryRunClearTokenPlansFieldDelete(t *testing.T) {
	setupAuth(t)
	t.Setenv("ASC_CONFIG_PATH", filepath.Join(t.TempDir(), "nonexistent.json"))
	t.Setenv("ASC_APP_ID", "")
	...
	if err := os.WriteFile(filepath.Join(dir, "app-info", "en-US.json"), []byte(`{"name":"App Name","subtitle":"__ASC_DELETE__"}`), 0o644); err != nil {
		t.Fatalf("write app-info file: %v", err)
	}
	...
	if !found {
		t.Fatalf("expected explicit clear token to plan subtitle delete, got %+v", payload.Deletes)
	}
}
...
func TestMetadataPushApplyClearTokenRecreatesLocalization(t *testing.T) {
	setupAuth(t)
	t.Setenv("ASC_CONFIG_PATH", filepath.Join(t.TempDir(), "nonexistent.json"))
	t.Setenv("ASC_APP_ID", "")
	...
	http.DefaultTransport = roundTripFunc(func(req *http.Request) (*http.Response, error) {
		...
		case "/v1/appInfoLocalizations":
			if req.Method == http.MethodPost {
				createCount++
				body := `{"data":{"type":"appInfoLocalizations","id":"loc-app-new","attributes":{"locale":"en-US","name":"App Name"}}}`
				return &http.Response{
					StatusCode: http.StatusCreated,
					Body:       io.NopCloser(strings.NewReader(body)),
					Header:     http.Header{"Content-Type": []string{"application/json"}},
	...
	if deleteCount != 1 || createCount != 1 {
		t.Fatalf("expected one delete and one create, got delete=%d create=%d", deleteCount, createCount)
	}
